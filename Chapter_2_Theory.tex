...
Continious variable quantum states are classified into two catogories:Gaussian and non Guassian states.\cite{AdessoRagyLee2014}
\subsection{Guassian states}
In a continious variable system quantum system,a Guassian state is a special kind of quantum state whose fourier representation has a guassian form in phase space.These states are completely described by their first and second moments.
...
\begin{equation}
	[X, P] = i
\end{equation} \cite{Serafini2017}

The weigner function of a guassian state is

\begin{equation}
	W(\boldsymbol{\xi}) = \frac{1}{(2\pi)^{n}\sqrt{\det\boldsymbol{\Upsilon}}}
	\exp\!\left[-\frac{1}{2}(\boldsymbol{\xi} - \bar{\boldsymbol{\xi}})^{T}
	\boldsymbol{\Upsilon}^{-1}(\boldsymbol{\xi} - \bar{\boldsymbol{\xi}})\right]
\end{equation}

\noindent
where 
\[
\boldsymbol{\xi} = (X_{1}, P_{1}, \ldots, X_{n}, P_{n})^{T}
\]
represents the phase-space vector, 
\[
\bar{\boldsymbol{\xi}} = \langle \hat{\boldsymbol{\xi}} \rangle
\]
is the displacement vector, and 
\[
\boldsymbol{\Upsilon}_{ij} = \frac{1}{2}
\langle \hat{\xi}_{i}\hat{\xi}_{j} + \hat{\xi}_{j}\hat{\xi}_{i} \rangle
- \langle \hat{\xi}_{i} \rangle \langle \hat{\xi}_{j} \rangle
\]


is the covariance matrix.\cite{WeedbrookEtAl2012} \cite{AdessoRagyLee2014}




All physical properties of the guassian states such as purity,entropy,steering and entanglement can be determined directly from the covariance matrix without full density operator because of the guassian form of weigner function.
typical examples of guassian states include the coherent states, squeezed states, thermal states and vaccume state.
...
For Gaussian states with covariance matrix $V$, the steering criterion derived from the PPT (Positive Partial Transpose) condition is \cite{DrShskitullshPHDPaper2015}:
\begin{equation}
...
Previous work on coupled lossy waveguides \cite{DrShskitullshPHDPaper2015} demonstrated:
